...
How to netboot the TC8 panel: u-boot pulls a kernel and DTB over TFTP, and the kernel mounts its rootfs over NFSv3. Result: the same fullscreen Wayland kiosk as the eMMC target, but nothing is written to the device's flash.
